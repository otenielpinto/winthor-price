/**
 * Migration script to create the produto_preco_old table in SQLite
...
/**
 * Creates the produto_preco_old table
 */
export async function up() {
  const knex = sql3Client();

  // Check if table already exists
  const tableExists = await knex.schema.hasTable("produto_preco_old");

  if (!tableExists) {
    console.log("Creating produto_preco_old table...");

    await knex.schema.createTable("produto_preco_old", (table) => {
    ...
    console.log("produto_preco_old table created successfully");
  } else {
    console.log("produto_preco_old table already exists");
  }
}

/**
 * Drops the produto_preco_old table
 */
export async function down() {
  const knex = sql3Client();
  return knex.schema.dropTableIfExists("produto_preco_old");
}
